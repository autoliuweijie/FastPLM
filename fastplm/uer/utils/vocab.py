...
class Vocab(object):
    ...
    def load(self, vocab_path, is_quiet=False):
        ...
        try:
            self.PAD_ID = self.w2i.get('<pad>', 0) + self.w2i.get('[PAD]', 0) 
            self.UNK_ID = self.w2i.get('<unk>', 0) + self.w2i.get('[UNK]', 0) 
            self.CLS_ID = self.w2i['[CLS]']
            self.SEP_ID = self.w2i['[SEP]']
            self.MASK_ID = self.w2i['[MASK]']
        except:
            raise Exception('Special token words not in vocabulary.')
    ...
    def get(self, w):
        return self.w2i.get(w, self.UNK_ID)
